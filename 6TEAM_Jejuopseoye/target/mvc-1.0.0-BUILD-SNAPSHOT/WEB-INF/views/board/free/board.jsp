...
<section class="py-6">
			...
			<c:if test="${not empty list}">
				<c:forEach var="board" items="${list}">
					...
					<td class="py-4 ps-5 align-middle">
						<a class="custom-a" href="${path}/board/free/board-detail?no=${board.no}"><c:out value="${board.title}"/></a>
					</td>
	...
	<nav aria-label="Page navigation example">
				...
				<li class="page-item">
				<button class="page-link" onclick="movePage('${path}/board/free/board?page=${pageInfo.prevPage}')">
				...
				<c:forEach begin="${pageInfo.startPage}" end="${pageInfo.endPage}" step="1" varStatus="status">
					...
					<c:if test="${status.current != pageInfo.currentPage}">
						<li class="page-item">
						<button class="page-link" onclick="movePage('${path}/board/free/board?page=${status.current}')">
				...
				<!-- 다음 페이지 -->
				<li class="page-item">
				<button class="page-link" onclick="movePage('${path}/board/free/board?page=${pageInfo.nextPage}')"><i class="fa fa-angle-right"></i></button>
				</li>
				</ul>
	</nav>
	</div>
	...
	<c:if test="${loginMember != null}">
		<div class="col-sm-11 pe-4" style="text-align: right;">
			<a href="${path}/board/free/board-write"><button class="btn btn-primary" type="submit">글쓰기</button></a>
		</div>
	</c:if>
	<!-- 검색바 -->
	<div class="row">
		<div class="col-xl-4 mx-auto">
			<form action="${path}/board/free/board" method="get">
				<div class="input-group">
					...
					<input type="hidden" name="typeType" id="typeType" value="${type}">
