...
<section class="py-6">
	<div class="container">
		...
		<div class="table-responsive">
			<table
				...
				<tr>
					...
					<td class="py-4 text-center align-middle" style="width: 80%"><c:out value="${board.title}"/></td>
				</tr>
				<tr>
					<th class="py-4 text-center align-middle">내용</th>
					<td class="py-4 text-center align-middle">
						<div style="min-height: 400px;"><c:out value="${board.content}"/></div>
					</td>
				</tr>
				<tr>
					<th class="py-4 text-center align-middle">작성자</th>
					<td class="py-4 text-center align-middle"><c:out value="${board.nickName}"/></td>
				</tr>
				<tr>
					<th class="py-4 text-center align-middle">작성시간</th>
					<td class="py-4 text-center align-middle"><fmt:formatDate type="both" value="${board.createDate}"/></td>
				</tr>
				<tr>
		...
		<div class="text-center align-middle mb-3">
			<a href="${path}/board/free/board">
				<button class="btn btn-primary" type="button">목록</button></a>
				<c:if test="${not empty loginMember && (loginMember.nickName == board.nickName
									|| loginMember.role == 'ROLE_ADMIN')}">
					<button style="margin-left: 20px;" class="btn btn-primary" type="button" id="btnUpdate">수정</button>
					<button style="margin-left: 20px;" class="btn btn-dark" type="button" id="btnDelete">삭제</button>
		 ...
		 <!-- 댓글 시작-->
			...
			<c:forEach var="reply" items="${replyList}">
				<div class="d-flex d-block d-sm-flex review">
                  ...
                  <div class="ms-auto">
                  <c:if test="${ !empty loginMember && (loginMember.nickName == reply.nickName 	|| loginMember.role == 'ROLE_ADMIN') }">
                    <button onclick="deleteReply('${reply.no}','${board.no}');" class="btn text-white border-start-0" type="button"
		...
		<c:if test="${empty replyList}">
			<div class="d-flex d-block d-sm-flex review">
              ...
              <form action="${path}/board/free/reply" method="post">
                  <input type="hidden" name="boardNo" value="${board.no}"/>
    			<input type="hidden" name="nickName" value="${loginMember.nickName}"/>
                <input style="width: 1280px; height: 100px" name="content" class="form-control form-control-lg bg-transparent border-1 border-end-0" type="text"
                  placeholder="댓글 남기기">
                <button class="btn text-white border-start-0 fs-6" type="submit"
                  style="background-color: rgb(107, 99, 99); float: right; margin-top: 10px">&nbsp; 등록&nbsp;</button>
                  </form>
              </div>
...
<script>
	$(document).ready(() => {
		$("#btnUpdate").on("click", (e) => {
			location.href = "${path}/board/free/board-update?no=${board.no}";
		});
		
		$("#btnDelete").on("click", (e) => {
			if(confirm("정말로 게시글을 삭제 하시겠습니까?")) {
				location.replace("${path}/board/free/delete?boardNo=${board.no}");
			}
		});
	});
	
	function deleteReply(replyNo, boardNo){
		var url = "${path}/board/free/replyDel?replyNo=";
