...
<jsp:include page="/WEB-INF/views/common/header.jsp">
	...
	<c:if test="${not empty list}">
		<c:forEach var="content" items="${list}">
			<c:set var="title" value="${content.contentscdLabel}"/>
			<c:set var="value" value="${content.contentscdValue}"/>
		</c:forEach>
	</c:if>

    <section class="hero-web d-flex align-items-end dark-overlay bg-cover"
        style="background-image: url(${path}/resources/img/photo/hero_bg2.png); background-size: 100% 100%;">
        <div class="container h-100">
            <div class="w-100 text-start position-absolute bottom-0 start-18 overlay-content">
                <div class="row">
                    <div class="col-xl-11">
                        <h1 class="display-3 font-weight-bold text-shadow" style="color: white;">${title}</h1>
                        <p class="text-lg text-shadow" style="color: white;">환상적이고 신비로운 섬, 제주로 놀러오세요!!!</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="py-5">
        <div class="container">
            <div class="row mb-5 searchbarlocation">
                <div class="col-xl-6 mx-auto">
                    <form action="${path}/board/content/attraction" method="get">
                        <div class="input-group">
                        	<input type="hidden" name="searchType" value="title">
                        	<input type="hidden" name="optionType" value="option">
                        	<input type="hidden" name="optionValue" id="optionValue" value="${value}">
                        	<input style="border-radius: 8px 0 0 8px/8px 0 0 8px" class="form-control text-lg" type="text" value="${param.searchValue}" id="searchValue" name="searchValue" placeholder="검색어 입력...">
                        	<button class="btn btn-primary text-xl" type="submit"><i class="fa fa-search"></i></button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="col-sm-6 col-xl-4 mb-5 hover-animate" data-marker-id="59c0c8e33b1527bfe2abaf92">
                    <div class="card h-100 border-0 shadow">
                        <div class="card-img-top overflow-hidden dark-overlay bg-cover"
                            ...
                            <a class="tile-link" href="${path}/board/content/board-detail?no=${content.no}"></a>
                        </div>
                        <div class="card-body">
                            ...
                            <p class="text-lg text-muted mb-1">
                            	<a href="${path}/board/content/board-detail?no=${content.no}" class="text-dark">
                            	<c:choose>
        							<c:when test="${fn:length(content.introduction) gt 22}">
        								<c:out value="${fn:substring(content.introduction, 0, 21)}...">
        								</c:out></c:when>
        							<c:otherwise>
                            ...
                            <p class="text-base text-end mb-0"><button class="btn btn-primary"  id="likebtn1" onclick="onClickLike1('likebtn1')"> 좋아요</button></p>
                        </div>
                    </div>
                </div>
                	</c:forEach>
			...
			<nav aria-label="Page navigation example">
				...
				<li class="page-item">
				<button class="page-link" onclick="movePage('${path}/board/content/attraction?page=${pageInfo.prevPage}')">
				...
				<c:forEach begin="${pageInfo.startPage}" end="${pageInfo.endPage}" step="1" varStatus="status">
					...
					<c:if test="${status.current != pageInfo.currentPage}">
						<li class="page-item">
						<button class="page-link" onclick="movePage('${path}/board/content/attraction?page=${status.current}')">
				...
				<!-- 다음 페이지 -->
				<li class="page-item">
				<button class="page-link" onclick="movePage('${path}/board/content/attraction?page=${pageInfo.nextPage}')"><i class="fa fa-angle-right"></i></button>
				</li>
				</ul>
	</nav>
	</div>
	...
	function movePage(pageUrl){
		...
		var optionType = document.getElementsByName("optionType"); // title
		...
		if(searchValue.value.length > 0){
			...
			pageUrl = pageUrl + '&searchType=' + searchType + '&searchValue=' + searchValue.value + '&optionType' + optionType + '&optionValue' + optionValue; 
		}
